...
async function createWindow() {
  ...
  try{
    var r = await sqlite.run(`
      CREATE TABLE print_records (
          ...
          task_id TEXT NOT NULL,             -- 打印任务uuid
          name TEXT NOT NULL,                     -- 打印文件名称
          templateId TEXT NOT NULL,          -- 打印模板名称
          print_time TEXT, -- 打印时间，格式为yyyy-MM-dd HH:mm:ss
          print_status TEXT NOT NULL CHECK(print_status IN ('pending', 'printing', 'completed', 'failed')), -- 打印状态，限定为几个可能的值
          msg TEXT          -- 备注
      );
    `)
    if(r){
      console.log("print_records created")
    }
  }catch(e){
    console.log(e)
  }
  ...
  MAIN_WINDOW.webContents.on("dom-ready", async () => {
    try {
      // 本地服务开启端口监听
      appServer.listen(store.get("port") || 17521);
      // 初始化本地 服务端事件
      initServeEvent(appServer);
      // 有配置中转服务时连接中转服务
      if (
